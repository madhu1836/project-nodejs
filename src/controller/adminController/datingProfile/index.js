...
module.exports={
    ...
    //     let reqObj = req.body;
    //     try {
    //         // let getProfileDetailsByQuery = await userDbHandler.getProfileDetailsByQuery({user_id:{$ne: id}});
    ...
    //         //     return responseHelper.success(res, responseData);
    //         // }
    //         let newProfile = await userDbHandler.createProfile(Data);
    ...
    updateDatingProfile: async (req, res) => {
        ...
        let reqObj = req.body;
        try {
            let getProfileDetailsByQuery = await userDbHandler.getUserDetailsByQuery({user_id: {$ne: id}});
            if (!getProfileDetailsByQuery.length) {
            ...
            //     return responseHelper.success(res, responseData);
            // }
            let updatingData = await userDbHandler.updateUserDetailsById(id, updateData);
            // let updatedData = await userDbHandler.getProfileDetailsByQuery({ profile_email: reqObj.profile_email })
    ...
    getDatingProfilesByFilter: async (req, res) => {
        ...
        let reqObj = req.body;
        try {  
            let getProfileList = await userDbHandler.getUserDetailsByQuery({ $and: [ {age: reqObj.age}, {height: reqObj.height},{weight: reqObj.weight} ] })
    ...
    getAllProfiles: async (req, res) => {
        ...
        let id = admin.sub;
        try {
            let getProfileList = await userDbHandler.getUserDetailsByQuery({})
            if (!getProfileList.length) {
    ...
    getSingleProfileById: async (req, res) => { 
        ...
        let id = req.params.id;
        try {
            let getProfile = await userDbHandler.getUserDetailsById(id);
            if (!getProfile) {
    ...
    getAllProfilesByGender: async (req, res) => {
        ...
        try {
            ...
            let getProfileList = await userDbHandler.getUserDetailsByQuery({ gender: reqObj.gender })
            if (!getProfileList.length) {
                responseData.msg = "Dating profiles does not exist";
                return responseHelper.error(res, responseData);
            }
    ...
    deleteDatingProfile: async (req, res) => {
        ...
        try {
            ...
            let deleteProfile= await userDbHandler.deleteUserById(id);
